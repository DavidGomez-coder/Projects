 ...
 */public class LinearProbingHashTable<K,V> implements HashTable<K,V>{
	...
	private int searchIdx(K key) {
		//posición en la que debería de estar el elemento
		int index = hash(key) % keys.length;
		//recorremos el array keys
		while (keys[index]!=null && !keys[index].equals(key)){
			//incrementamos el índice en módulo la longitud del array (circular)
	...
	public V search(K key) {
		//buscamos el índice en el que debería de estar ese valor y devolvemos
	...
	public void insert(K key, V value) {
		...
		//obtenemos el índice donde deberia de guardarse el valor
		int index = searchIdx(key);
	...
	public void delete(K key) {
		//posición donde debería de estar el elemento
		int index = searchIdx(key);
		if (keys[index] != null) {
			keys[index] = null;
			values[index] = null;
	...
	private class TableIter {
		...
		public void advance() {
			...
			//incrementamos el número de nodos visitados
			visited++;
		}
