 ...
 */public class LinearProbingHashTable<K,V> implements HashTable<K,V>{
	...
	private int searchIdx(K key) {
		//calculamos la posición donde debería de estar el elemento
		//teniendo en cuenta que es un array circular
		int index = hash(key) % keys.length;
		//recorremos el array de claves mientras el objeto en esta posición no sea nulo y distinto de key
	...
	public void insert(K key, V value) {
		...
		//índice en el que debería de estar
		int index = searchIdx(key);
		//si la clave no existe, entonces la insertamos e incrementamos el tamaño de
		//la tabla
		if (keys[index]==null){
			keys[index] = key;
			size++;
		}
		//actualizamos el valor asociado a la clave
		values[index] = value;
	}
	...
	public void delete(K key) {
		//indice donde debería de estar la clave
		int index = searchIdx(key);
		if (keys[index]!=null){
			//eliminamos asociación
			keys[index] = null;
			values[index] = null;
			//hacemos una nueva reinserción de todos los valores
			int i = index +1;
	...
	/**
	 * Método auxiliar para eliminar y reinsertar un par clave-valor situado en la posición i
	 */
	private void deleteAndReinsert (int i){
		K k_aux = keys[i];
		V v_aux = values[i];
		keys[i] = null;
		values[i] = null;
