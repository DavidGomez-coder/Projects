...
void manejadorZombies(){
	...
	for (int i=1;i<=list_size(procesos);i++){
		...
		pid_wait = waitpid(trabajo->pgid,&status, WNOHANG); //obtenemos el pid del proceso por el que se envía la señal (pid no bloqueante)
	...
	if (pid_fork < 0){ //ha ocurrido un error
		...
		exit(-1);
        	...
        	if (background == 0){ //esta en primer plano
				block_SIGCHLD();
				add_job(procesos,new_job(pid_fork,args[0],PRIMERPLANO));
				unblock_SIGCHLD();
				//esperamos al hijo	
			...
			block_SIGCHLD();
			add_job(procesos,new_job(pid_fork,args[0],SEGUNDOPLANO));
